...
1170  FORMAT(/' Done.'/)
     ...
     *PARANOT2,LPARANINC1,LPARANINC2,LNINC,NHSTRY, max_total_particle_no
...
1930  FORMAT(/' BEGIN READING/WRITING PH-SP DATA .....'/)
      ...
      IF ((i_iaea_in.EQ.0 .AND. PARANOT1+PARANOT2.GT.max_total_particle_
     *no)) THEN
        WRITE(6,1940)
1940    FORMAT(//' ***WARNING***'/ ' Total no. of particles in combined'
     *, ' file will exceed 2^32-1.'/ ' This number cannot be stored inth
     *e header.'/ ' Exiting now.'//)
        sumerr=2
        return
      END IF
      IF ((i_iaea_in.EQ.1)) THEN
